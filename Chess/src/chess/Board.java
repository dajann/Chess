...
public class Board extends JComponent {
    ...
    public void initGrid()
    {
        ...
        Black_Pieces.add(new King(4,7,false,"King.png",this));
        Black_Pieces.add(new Queen(3,7,false,"Queen.png",this));
    ...
    public boolean isCheck(boolean isWhite, int x , int y, int destination_x, int destination_y) {
		...
		Piece temp_dest = getPiece(destination_x, destination_y);
    	...
    	if(isWhite) {
    		Piece piece = White_Pieces.get(0);
    		for(int i = 0; i < Black_Pieces.size(); i++) {
    			if(Black_Pieces.get(i).getX() != -1)
    				if(Black_Pieces.get(i).canMove(piece.getX(), piece.getY())) {
    					temp.setX(x);
    					temp.setY(y);
    					if(temp_dest != null) {
    		...
    		Piece piece = Black_Pieces.get(0);
    		for(int i = 0; i < White_Pieces.size(); i++) {
    			if(White_Pieces.get(i).getX() != -1)
    				if(White_Pieces.get(i).canMove(piece.getX(), piece.getY())){
    					temp.setX(x);
    					temp.setY(y);
    					if(temp_dest != null) {
    						temp_dest.setX(destination_x);
    ...
    public boolean isGameOver(boolean isWhite) {
		...
		for(int i = 0; i < Black_Pieces.size(); i++) {
				for(int j = 0; j < 8; j++)
					for(int z = 0; z < 8; z++) {
						if(Black_Pieces.get(i).canMove(j, z)) return false;
						}
				}
    	}
    	else {
    		for(int i = 0; i < White_Pieces.size(); i++) {
				for(int j = 0; j < 8; j++)
					for(int z = 0; z < 8; z++) {
						if(White_Pieces.get(i).canMove(j, z)) return false;
						}
				}
    	}
    	return true;
        ...
        public void mousePressed(MouseEvent e) {
            ...
            else if (Active_Piece != null && Active_Piece.getX() == Clicked_Column && Active_Piece.getY() == Clicked_Row)
            {
                Active_Piece = null;
            		...
            		!isCheck(Active_Piece.isWhite() ,Active_Piece.getX(), Active_Piece.getY(), Clicked_Column, Clicked_Row)
                    && ((is_whites_turn && Active_Piece.isWhite()) || (!is_whites_turn && Active_Piece.isBlack())))
            {
                ...
                isGameOver(Active_Piece.isWhite());
                Active_Piece = null;
                turnCounter++;
            }
